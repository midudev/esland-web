import {
  getAwardImageURL,
  getImageURL,
  getMobileAwardImageURL,
  getThumbURL
} from './src/all-images-urls-functions'
...
const downloadFinalistImage = async (
  photo: string,
  edition: '1' | '2',
  basePath: string
) => {
...
for (const { edition: e, maxPhotos, info } of editionsInfo) {
  ...
  for (const photoNumber in Array.from({ length: maxPhotos + 1 })) {
    ...
    if (isValidThumb) {
      ...
      await sharp(await downloadImage(thumbnailUrl))
        .webp({
          quality: 100,
          lossless: true
        })
    ...
    if (isValidImg) {
      ...
      await sharp(await downloadImage(imageUrl))
        .webp({
          quality: 100,
          lossless: true
        })

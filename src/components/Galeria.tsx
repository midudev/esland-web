...
export default function Galeria({ edicion }: { edicion: string }) {
  ...
  const handleLoadMore = async (e: MouseEvent) => {
    ...
    const html = images
      .map((img: any, index: number) => {
        ...
        clone.setAttribute("href", `/archivo-page/${edicion}/gallery/img-${imgIndex}.webp`);
        clone.classList.add("animate-fade-up");
        clone.classList.add("animate-delay-300");
        clone.classList.add("opacity-0");
        clone.querySelector("img:first-child")?.setAttribute("src", `/archivo-page/${edicion}/gallery/thumbnails/img-${imgIndex}.webp`);
        clone.querySelector("img:last-child")?.setAttribute("src", `/archivo-page/${edicion}/gallery/thumbnails/img-${imgIndex}.webp`);
  ...
  return (
    <section class="max-w-8xl mx-auto py-20 px-20">
      <Title tag="h2" space="space-small" weight="weight-bold" center secondaryText={i18n.GALLERY.TEXT}>
        {i18n.GALLERY.TITLE}
      </Title>

      <masonry-layout gap="24" maxcolwidth="600" class="lg:mx-auto mx-4 py-20" id="gallery">

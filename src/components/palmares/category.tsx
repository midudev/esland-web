...
export const Category = ({ info, edition, isMobile = false, open = false, onClick }: CategoryCardProps) => {
      ...
      <article className={Styles.container} onClick={onClick}>
        ...
        {isOpen && isMobile && (
          <section ref={cardsRef} style={{ height }}>
            <div>
              <Card edition={edition} position={1} info={info} isMobile={isMobile} />
              <Card edition={edition} position={2} info={info} isMobile={isMobile} />
            </div>
          </section>
        )}
      </article>

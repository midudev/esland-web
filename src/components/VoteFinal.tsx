...
export const VoteFinal = ({
  ...
  const selectedCandidates = useMemo(() => {
    ...
    votes.flat(2).forEach((voteId) => {
      ...
      const categoryInfo = candidates.find(
        (category) => category.id === categoryId
      )
      const candidateInfo = categoryInfo?.candidatos?.find(
  ...
  return (
    <section class='flex flex-col'>
      {result === RESULT_STATUS.SUCCESS && (
        <h1 class='mx-auto text-balance pb-10 pt-24 text-left text-3xl font-semibold tracking-wide lg:text-5xl'>
          Gracias por votar
        </h1>
      )}
      ...
      {(result === RESULT_STATUS.IDLE || isLoading) && (
        <>
          ...
          <div
            ...
            } grid grid-cols-6 gap-2 transition`}
          >
            {votes.map((categoryVotes, index) => {
              return (
                <button
                  class='flex flex-col items-center justify-between bg-[#1682c7] hover:scale-105 hover:bg-[#54beff]'
                ...
                >
                  ...
                  <h2 class='my-auto text-balance text-center text-lg font-semibold'>
                    {categoryNames[index]}
                  </h2>
                </button>
              )
          ...
          <div class='mb-24 flex w-full flex-col items-center justify-center gap-y-4 py-10'>
            <button
              class={`${
                result === RESULT_STATUS.LOADING ? 'disabled opacity-90' : ''
              } animate-fade-up rounded-full bg-white px-5 py-3 font-medium uppercase text-blue-950 no-underline transition hover:scale-125 hover:border-yellow-400 hover:bg-yellow-300 lg:text-2xl`}
              onClick={handleSubmit}
            >
              {result === RESULT_STATUS.LOADING && (
                <svg
                  aria-hidden='true'
                  class='h-8 w-8 animate-spin text-gray-600'
                  viewBox='0 0 100 101'
                  fill='none'
                >
                  <path
              ...
              {result === RESULT_STATUS.IDLE && 'Enviar mis votos'}
            </button>
            <button
              class='animate-fade transition animate-delay-200 hover:text-yellow-200 hover:underline'
